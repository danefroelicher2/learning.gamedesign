...
class GameStateManager {
  ...
  updateUI() {
    ...
    switch (this.currentState) {
      case "worldMap":
        ...
        if (controls)
          controls.textContent = "Use A/D to move, Enter/Space to enter level";
        break;
  ...
  render(ctx, canvas) {
    switch (this.currentState) {
      ...
      case "playing":
        // Apply camera transformation for level rendering
        camera.apply(ctx);
      ...
      case "levelComplete":
        // Keep showing the level with camera
        camera.apply(ctx);
        levelManager.render(ctx, canvas);
        player.render(ctx);
        camera.restore(ctx);

        // Overlay completion message (no camera transform for UI)
