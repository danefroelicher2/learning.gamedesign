...
class Player {
    ...
    respawn() {
        ...
        // ALWAYS respawn at level start - no special boss arena respawn
        this.setPosition(level.playerStart.x, level.playerStart.y);
        
        // Reset boss arena state when player dies and respawns
        if (level.bossArena && level.bossArena.triggered) {
            level.bossArena.triggered = false; // Reset boss arena trigger
            camera.unlockFromBossArena(); // Unlock camera
            levelManager.bossDefeated = false; // Reset boss defeated state
        }
    ...
    checkBossArenaBounds(canvas) {
        const level = levelManager.getCurrentLevel();
        if (!level || !level.bossArena || !level.bossArena.triggered || levelManager.bossDefeated) return;
...
checkCannoneerCollisions() {
        ...
        for (let mob of mobs) {
            if (mob.active && (mob.type === 'cannoneer' || mob.type === 'spikeRoller')) {
        ...
        for (let mob of mobs) {
            if (mob.active && mob.type === 'ironGiant') {
                ...
                if (this.velocityY > 0 && this.checkCollision(mob)) {
                    ...
                    if (playerBottom <= bossTop + 10 && Math.abs(playerCenter - bossCenter) < mob.width / 2) {
