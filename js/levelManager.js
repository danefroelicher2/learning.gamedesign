    ...
    resetProgressOnRespawn(respawnType) {
        ...
        switch(respawnType) {
            case 'start':
                ...
                break;
            case 'checkpoint1':
                this.currentProgress = 15; // After tutorial section
                console.log('Progress reset to 15% (checkpoint 1)');
                break;
            case 'checkpoint2':
                this.currentProgress = 25; // After precision gauntlet
                console.log('Progress reset to 25% (checkpoint 2)');
                break;
            case 'checkpoint3':
                this.currentProgress = 40; // After spike introduction
                console.log('Progress reset to 40% (checkpoint 3)');
                break;
            case 'checkpoint4':
                this.currentProgress = 65; // After descent
                console.log('Progress reset to 65% (checkpoint 4)');
                break;
            case 'checkpoint5':
                this.currentProgress = 80; // After spike gauntlet
                console.log('Progress reset to 80% (checkpoint 5)');
                break;
            case 'bossArena':
                this.currentProgress = 90; // Boss arena
    ...
    drawCheckpoint(ctx) {
        ...
        for (let collectible of level.collectibles) {
            if (collectible.type === 'checkpoint') {
                // Determine if this checkpoint has been reached based on position
                let isReached = false;
                if (collectible.x < 1500) {
                    isReached = level.checkpoint1Reached;
                } else if (collectible.x < 3000) {
                    isReached = level.checkpoint2Reached;
                } else if (collectible.x < 5000) {
                    isReached = level.checkpoint3Reached;
                } else if (collectible.x < 8000) {
                    isReached = level.checkpoint4Reached;
                } else {
                    isReached = level.checkpoint5Reached;
                }
