...
class LevelManager {
  ...
  updateProgress() {
    ...
    const positions = Object.keys(markers)
      .map((x) => parseInt(x))
      .sort((a, b) => a - b);
    ...
    for (let i = 0; i < positions.length - 1; i++) {
      ...
      if (playerX >= currentPos && playerX < nextPos) {
        ...
        newProgress =
          markers[currentPos] + progressRange * (playerOffset / positionRange);
        break;
      }
    }
  ...
  // NEW: Draw progress bar at top of screen
  drawProgressBar(ctx, canvas) {
    ...
    ctx.fillText(progressText, canvas.width / 2, barY + 15);
    ctx.textAlign = "left";
  ...
  drawCheckpoint(ctx) {
    ...
    for (let collectible of level.collectibles) {
      if (collectible.type === "checkpoint") {
        ...
        // Pole base
        ctx.fillRect(
          collectible.x + collectible.width / 2 - 6,
          collectible.y + collectible.height + 16,
          12,
          4
        );
